...
def _notify(user, message):
    """
    Stub — identical pattern to every other app per handoff v7 §19
    ("All notification calls in services are currently stubs").
    Month 3: replace with a Celery task calling Arkesel.
    """
    import logging

    logging.getLogger(__name__).debug(
        "NOTIFY %s: %s", getattr(user, "phone_number", user), message
    )
